...
<div {{ $attributes->merge(['class' => "relative inline-block $sizeClass"]) }}>
    <img src="{{ $src ?? 'https://ui-avatars.com/api/?name=' . urlencode($name ?? 'User') . '&color=0A1647&background=D4AF37&bold=true&size=200' }}" 
         alt="{{ $name ?? 'Avatar' }}" 
         class="rounded-full object-cover w-full h-full ring-2 ring-vm-gold shadow-lg"
         onerror="this.src='https://ui-avatars.com/api/?name={{ urlencode($name ?? 'User') }}&color=0A1647&background=D4AF37&bold=true&size=200'">
